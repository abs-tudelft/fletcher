...
std::shared_ptr<Type> cmd_type(const std::shared_ptr<Node> &index_width,
  ...
  // If we want the ctrl field to be visible on this type, create that as well.
  // This field is used to pass addresses. Depending on how advanced the developer is,
  // we want to expose this or leave it out through the Nucleus layer.
  if (ctrl_width) {
 ...
 *
 * Changes to the implementation of this component in the HDL source must be reflected in
 * the implementation of this function.
 *
 * WARNING: Binding of the input/output data stream width generics is more arcane than
 *  what is good for most. As such, most widths are just bound to some integer literals
 *  rather than parameters. Any code instantiating this component should rebind the type
 *  themselves after figuring out their true width.
 *
...
Component *array(Mode mode) {
  ...
  // Command port.
  auto cmd = port("cmd",
                  cmd_type(iw, tw, strl("arcfg_ctrlWidth(CFG, BUS_ADDR_WIDTH)")),
                  Port::Dir::IN,
                  kernel_cd());
...
ConfigType GetConfigType(const arrow::DataType &type) {
  if (type.id() == arrow::Type::LIST) {
    ...
    if (!type.field(0)->nullable()
        && (GetConfigType(*type.field(0)->type()) == ConfigType::PRIM)) {
...
std::shared_ptr<Node> GetWidthNode(const arrow::DataType &type) {
  switch (type.id()) {
    ...
    case arrow::Type::FIXED_SIZE_LIST: {
      auto t = std::static_pointer_cast<arrow::PrimitiveCType>(type.field(0)->type());
      return intl(t->bit_width());
    }
...
std::string GenerateConfigString(const arrow::Field &field, int level) {
  ...
  if (ct == ConfigType::PRIM) {
    ...
    ret += "listprim(";
    level++;
    // Binary and string have no child, so we can't inspect it for the width, which is
    // always 8.
...
std::shared_ptr<Type> ListPrimType(int val_epc,
                                   int list_epc,
                                   int val_width,
                                   int idx_width,
...
std::shared_ptr<Type> GetStreamType(const arrow::Field &arrow_field,
                                    fletcher::Mode mode,
                                    int level) {
  // The ordering of the record fields in this function determines the order in which a
  // nested stream is type converted automatically using GetStreamTypeConverter. This
  // corresponds to how the hardware is implemented. More specifically, this is how the
  // data, count and validity fields are currently concatenated onto one big data field
  // of the output and input streams of ArrayReaders/Writers.
  //
  // WARNING: Modifications to this function must be reflected in the manual hardware
  //   implementation of Fletcher components! See: hardware/arrays/ArrayConfig_pkg.vhd
  ...
  switch (arrow_field.type()->id()) {
    ...
    case arrow::Type::BINARY:
      return ListPrimType(epc,
                          lepc,
                          8,
                          ARROW_OFFSET_WIDTH,
                          "bytes");
      // Special case: string type has a length stream and non-nullable utf8 character
      // stream. The EPC is assumed to relate to the list values. The LEPC can be used
      // for the length stream.
    ...
    case arrow::Type::STRING:
      ...
      // If the values are non-nullable primitives, we can use the "listprim"
      // configuration, which has some additional options.
    case arrow::Type::LIST: {
      ...
      if (GetConfigType(*child_field->type()) == ConfigType::PRIM) {
        ...
        FLETCHER_LOG(DEBUG,
                     "Using \"listprim\" configuration for list of non-nullable "
                     "primitives of width " << w);
        auto values_type =
            ConvertFixedWidthType(arrow_field.type()->field(0)->type(), epc);
        ...
        if ((epc > 1) || (lepc > 1)) {
          FLETCHER_LOG(FATAL,
                       "(Length)-elements-per-cycle > 1 on non-primitive list is not "
                       "supported.");
        }
    ...
    case arrow::Type::FIXED_SIZE_LIST: {
      // Inspect the item field.
      auto item_field = arrow_field.type()->field(0);
      // First handle the special case where this can be made a listprim
      if (GetConfigType(*item_field->type()) == ConfigType::PRIM) {
        type = ConvertFixedWidthType(arrow_field.type()->field(0)->type(), epc);
      } else {
        FLETCHER_LOG(ERROR,
                     "Fixed size lists with other than primitives is not supported.");
      }
      break;
    }
...
std::pair<uint32_t, uint32_t> GetArrayDataSpec(const arrow::Field &arrow_field) {
  ...
  switch (arrow_field.type()->id()) {
    case arrow::Type::BINARY: {
      ...
      return {2,
              e_count_width + l_count_width + data_width + length_width + validity_bit};
    }
    ...
    case arrow::Type::LIST: {
      ...
      if (GetConfigType(*child_field->type()) == ConfigType::PRIM) {
        ...
        return {2, e_count_width + l_count_width + data_width * epc
            + ARROW_OFFSET_WIDTH * lepc + validity_bit};
      } else {
        ...
        return {elem_spec.first + 1,
                elem_spec.second + ARROW_OFFSET_WIDTH + validity_bit};
      }
    }
    ...
    case arrow::Type::FIXED_SIZE_LIST: {
      auto item_field = arrow_field.type()->field(0);
      if (GetConfigType(*item_field->type()) == ConfigType::PRIM) {
        auto data_width = GetFixedWidthTypeBitWidth(*item_field->type());
        return {1, (epc > 1 ? e_count_width : 0) + epc * (data_width + validity_bit)};
      } else {
        FLETCHER_LOG(ERROR,
                     "Fixed size list with other than primitives is not supported.");
      }
    }
