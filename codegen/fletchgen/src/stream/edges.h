...
struct Edge : public Named {
  ...
  std::shared_ptr<Node> other(const std::shared_ptr<Node> &n) {
    if (src == n) return dst;
    if (dst == n) return src;
    else return nullptr;
  }

  static int TotalOf(const std::shared_ptr<Node> &node) {
    return static_cast<int>(node->ins.size() + node->outs.size());
  }

  static int IndexOf(const std::shared_ptr<Edge> &edge, const std::shared_ptr<Node> &node) {
    if (edge->src == node) {
      auto it = std::find(std::begin(node->ins), std::end(node->ins), edge);
      return static_cast<int>(std::distance(std::begin(node->ins), it));
    } else {
      auto it = std::find(std::begin(node->outs), std::end(node->outs), edge);
      return static_cast<int>(std::distance(std::begin(node->outs), it));
    }
  }
